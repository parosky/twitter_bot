...
class Api(object):
  ...
  # original
  def GetRetweetsToMe(self, count=None, since_id=None, max_id=None, include_entities=False):
     '''Fetch the sequence of retweets made by a single user.
  ...
  # original
  def makeRetweet(self, sid):
     url = '%s/statuses/retweet/%s.json' % (self.base_url, sid)
     if not self._oauth_consumer:
       raise TwitterError("The twitter.Api instance must be authenticated.")
     json = self._FetchUrl(url, post_data={"id":sid})
     return json
     data = self._ParseAndCheckTwitter(json)
     return [Status.NewFromJsonDict(x) for x in data]
  ...
  def FriendshipExists(self, user_a, user_b):
    # if a follows b
    url  = '%s/friendships/exists.json?user_a=%s&user_b=%s' % (self.base_url, user_a, user_b)
    json = self._FetchUrl(url)
    if json == "true":
        json = True
    else:
