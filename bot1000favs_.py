...
class Bot1000Favs(basebot.BaseBot):
    def __init__(self):
    ...
    def post(self, target):
        ...
        try:
            post_recentid = session.query(basebot.KeyValue).filter(basebot.KeyValue.key==db_key).one()
        except sqlalchemy.orm.exc.NoResultFound:
            post_recentid = basebot.KeyValue(db_key, 0)
